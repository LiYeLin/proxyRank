...
def create_speed_record(record: SSRSpeedTestRecord):
    ...
    try:
        cursor.execute('''
            INSERT INTO sr_speed_test_record (airport_id, airport_name, node_id, node_name,
...
def get_speed_record_by_id(record_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM sr_speed_test_record WHERE UniqueID = ?", (record_id,))
        row = cursor.fetchone()
        if row:
...
def delete_speed_record(record_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM sr_speed_test_record WHERE UniqueID = ?", (record_id,))
        conn.commit()
        return True
    except sqlite3.Error as e:
        print(f"删除测速记录失败: {e}")
